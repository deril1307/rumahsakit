...
<head>
    ...
    {{-- TAMBAHAN CSS: Untuk menghilangkan ikon mata bawaan browser --}}
    <style>
        /* Sembunyikan ikon mata bawaan Edge/IE */
        input[type="password"]::-ms-reveal,
        input[type="password"]::-ms-clear {
            display: none;
        }

        /* Sembunyikan ikon mata bawaan Chrome/browsers lain (jika ada) */
...
<body class="font-sans text-gray-900 antialiased bg-white">
    ...
    <div class="min-h-screen flex flex-col items-center justify-center pt-6 sm:pt-0">
        ...
        <div class="flex flex-col items-center mb-6">
            <div
                class="w-24 h-24 bg-white rounded-full shadow-sm flex items-center justify-center border border-gray-200 p-2">
        ...
        <div class="w-full sm:max-w-md px-6 py-4 bg-transparent">
            ...
            <form method="POST" action="{{ route('login') }}">
                ...
                {{-- Bagian Password yang Diperbaiki --}}
                <div class="mt-4" x-data="{ show: false }">
                    <label for="password" class="block font-bold text-sm text-gray-700 mb-1">Password</label>
                    <div class="relative">
                        <input id="password"
                            class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 py-3 px-4 text-gray-600 pr-10"
                            :type="show ? 'text' : 'password'" name="password" placeholder="Masukkan password Anda"
                ...
                <div class="flex items-center justify-end mt-2">
                    @if (Route::has('password.request'))
                        <a class="text-sm text-orange-400 hover:text-orange-500 font-semibold"
                ...
                <div class="mt-6">
                    <button
                        class="w-full bg-green-700 hover:bg-green-800 text-white font-bold py-3 px-4 rounded-md shadow transition duration-150 ease-in-out">
                        {{ __('Masuk') }}
                    </button>
                </div>
            </form>
